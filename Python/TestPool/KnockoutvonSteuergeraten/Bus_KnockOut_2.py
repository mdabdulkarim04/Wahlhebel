# ******************************************************************************
# -*- coding: latin1 -*-
# File    : Bus_KnockOut_2.py
# Title   : Bus_KnockOut_2.py
# Task    : Test for Bus KnockOut_2
#
...
# ------------------------------------------------------------------------------
# 1.0  | 10.02.2022 | Mohammed   | initial
# 1.1  | 24.02.2022 | Mohammed   | Added Fehler Id
# 1.2  | 25.02.2022 | Devangbhai | Rework
# 1.3  | 14.03.2022 | Mohammed   | Updated TestSpec ID and Name
# 1.3  | 14.03.2022 | Devangbhai | Exchange the test step 19 and 20
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_345")
    ...
    # test step 8
    testresult.append(["\x0a 8 Senden nur der NM Botschaft,NVEM12 und Clampcontrol von HIL--> ECU. Alle anderen Botschaften stoppen."])
    ...
    # test step 15
    testresult.append(["\x0a15 Prüfe NVEM_12:NVEM_Abschaltstufe != 3 (Stufe_3)"])
    testresult += [
        basic_tests.checkStatus(hil.NVEM_12__NVEM_Abschaltstufe__value, 3, equal=False,
                                descr="KN_Waehlhebel_Abschaltstufe != 3 (Stufe_3)"),
    ]
    # test step 16
    testresult.append(["\x0a16 Sende NVEM_12:NVEM_Abschaltstufe = 3 (Stufe_3) und warte 1s"])
    hil.NVEM_12__NVEM_Abschaltstufe__value.set(3)
    time.sleep(1)


    # test step 17
    testresult.append(["\x0a17 Prüfe NVEM_12:NVEM_Abschaltstufe = 1 (Stufe_3)"])
    testresult += [
       basic_tests.checkStatus(hil.KN_Waehlhebel__Waehlhebel_Abschaltstufe__value, 1,
                               descr="KN_Waehlhebel_Abschaltstufe = 1 (Funktionseinschraenkung)"),
    ]

    # testresult.append(_checkStatus(current_status=hil.NVEM_12__NVEM_Abschaltstufe__value,
    #                                nominal_status=1,
    #                                descr="KN_Waehlhebel_Abschaltstufe = 1 (Funktionseinschraenkung) ist",
    #                                ticket_id='FehlerId:EGA-PRM-149'))

    # teststep 18
    testresult.append(["\x0a18 Warte bis KN_Waehlhebel:KN_Waehlhebel_BUSKnockOutTimer == 0 (Timeout: 15min)"])
    # warte bis timer = 0
    timeout = 15* 60
    t_out = timeout + t()
    while t_out > t():
    ...
    # test step 19
    testresult.append(["\x0a 19. Prüfe KN_Waehlhebel:KN_Waehlhebel_BusKnockOut = 2 (0x2= Funktion_ausgeloest)"])
    testresult += [
    ...
    # Test step 20
    testresult.append(["\x0a 20. Schalte alle signal (HIL--> ECU) aus und Stoppe Measurement", ""])
    time.sleep(0.350)
    func_nm.hil_ecu_tx_signal_state_for_Knockout(NM_Clampcontrol_send=False, all_other_send=False)
    daq_data = daq.stopMeasurement()

    # teststep 21
    testresult.append(["\x0a 21. Warte 1 min. und Prüfe Busruhe"])
    time.sleep(60)
    descr, verdict = func_gs.checkBusruhe(daq, 1)
    testresult.append([descr, verdict])

    # test step 22
    testresult.append(["\x0a 22. Schalte alle signal (HIL--> ECU) ein und warte 5sec", ""])
    func_nm.hil_ecu_tx_signal_state_for_Knockout(NM_Clampcontrol_send=True, all_other_send=True)
    hil.cl15_on__.set(1)
    time.sleep(5)

    # teststep 23
    testresult.append(["\x0a 23 Prüfe BusKnockOut_Tmr == 16 bus_tmr "])
    request = [0x22] + diag_ident_KN_TMR['identifier']
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    testresult.append(["Auf positive Response überprüfen", ""])
    ...
    # test step 24
    testresult.append(["\x0a 24 Prüfe nichtflüchtige Speicherung von BusKnockOut_Ctr "])
    request = [0x22] + diag_ident_KN_CTR['identifier']
    ...
    # teststep 25
    testresult.append(["\x0a 25 Prüfe KN_Waehlhebel:KN_Waehlhebel_BusKnockOut = 2  (0x2= Funktion ausgeloest)"])
    ...
    # teststep 26
    testresult.append(["\x0a 26 Prüfe KN_Waehlhebel:KN_Waehlhebel_BusKnockOutTimer = 16(bus_tmr) == (Reset durch SG-Reset)"])
    testresult += [
        basic_tests.checkStatus(hil.KN_Waehlhebel__KN_Waehlhebel_BusKnockOutTimer__value, 16,
                                descr="KN_Waehlhebel_BusKnockOutTimer = 16"),
    ]
    ...
    # teststep 27, 28.1
    testresult.append(["\x0a 27. Start Analyse of DAQ Measurement", ""])
   ...
   #  # teststep 28.2
   #  testresult.append(["\x0a 28.2 Prüfe KN_Waehlhebel:KN_Waehlhebel_BusKnockOutTimer den Wert von 0 erreicht hat"])
   #  #testresult += [
   #   #   basic_tests.checkStatus(hil.KN_Waehlhebel__KN_Waehlhebel_BusKnockOutTimer__value, 0,
   #  #                            descr="KN_Waehlhebel_BusKnockOutTimer = 0"),
   # # ]
   #
   #  testresult.append(_checkStatus(current_status=hil.KN_Waehlhebel__KN_Waehlhebel_BusKnockOutTimer__value,
   #                                 nominal_status=0,
   #                                 descr="KN_Waehlhebel_BusKnockOutTimer = 0 ist",
   #                                 ticket_id='FehlerId:EGA-PRM-149'))
