...
# -*- coding: latin-1 -*-
#
# File    : STANDALONE_ACTIVE_Mode_1.py
# Title   : STANDALONE_ACTIVE_Mode_1
# Task    : STANDALONE ACTIVE in Mode 1
#           
# Author  : M.A. Mushtaq
# Date    : 23.02.2022
...
try:
    ...
    request_undefined_2 = [0x6E, 0xC1, 0x11, 0x11, 0x22, 0x33, 0x44]
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_283")
    ...
    for i in range(2):
        if i==0:
            testresult.append(["[-] 'Setze  Standalone Mode 1 inaktive und Mode 2 aktive", ""])
        else:
            testresult.append(["[-] 'Setze Standalone-Mode 1 und 2 Auf Standalone aktiv setzen", ""])
        ...
        testresult.append(["[-0]", ""])
        if i == 0:
            testresult.append(["[.] Schreiben des ECU Standalone-Modes 2: %s " %request_defined_2, ""])
            ...
            testresult.append(verdict)
            testresult.append(canape_diag.checkResponse(response, request_undefined_1))
            testresult.append(verdict)
        else:
            testresult.append(["[.] Schreiben des ECU Standalone-Modes 1: %s " % request_defined_1, ""])
            ...
            testresult.append(["[.] Auslesen Standalone_Modes_2: %s " % request_defined_2, ""])
            response, verdict = canape_diag.sendDiagRequest(request_defined_2)
            testresult.append(verdict)
            testresult.append(canape_diag.checkResponse(response, expected_response_2))
