...
# -*- coding: latin-1 -*-
#
# File    : CAN_Rx_Signals_ORU_Control_A_01.py
# Title   : CAN Rx Signals ORU Control A 01
# Task    : Test of HIL-Tx => ECU-Rx Signals of CAN Message ORU_Control_A_01
#
# Author  : A. Neumann
# Date    : 07.05.2021
...
try:
    ...
    # set Testcase ID #########################################################
    setTestcaseId(testresult, 'ORU_Control_A_01__OnlineRemoteUpdateControlOldA')
    testresult.append(["\nMessage/PDU: ORU_Control_A_01:OnlineRemoteUpdateControlOldA", ""]) 

    # #########################################################################
    # ORU_Control_A_01:OnlineRemoteUpdateControlOldA
    #
    ...
    testresult.append(testRxSigSeq(
        set_sig      = can_bus.ORU_Control_A_01__OnlineRemoteUpdateControlOldA__value,
        check_var    = cal.Sw_GSL_OTA_PIM_SG_ORU_Control_A_01_OnlineRemoteUpdateControlOldA, # Added correct cal-variable from .a2l
    ...
    # set Testcase ID #########################################################
    setTestcaseId(testresult, 'ORU_Control_A_01__ORU_Control_A_01_BZ')
    testresult.append(["\nMessage/PDU: ORU_Control_A_01:ORU_Control_A_01_BZ", ""]) 
    ...
    # Total range (raw+phys):    [0x0..0xF]
    testresult.append(testRxSigSeq(
        set_sig      = can_bus.ORU_Control_A_01__ORU_Control_A_01_BZ__value,
        check_var    = cal.Sw_GSL_OTA_PIM_SG_ORU_Control_A_01_ORU_Control_A_01_BZ, # Added correct cal-variable from .a2l
...
'''
    # set Testcase ID #########################################################
    setTestcaseId(testresult, 'ORU_Control_A_01__OnlineRemoteUpdateControlA')
    testresult.append(["\nMessage/PDU: ORU_Control_A_01:OnlineRemoteUpdateControlA", ""]) 

    # #########################################################################
    # ORU_Control_A_01:OnlineRemoteUpdateControlA
    #
    # Bit weight (resolution):   1/bit
    # Bit length:                4 bits
    # Lookup (raw):
    ...
    #   0x6: PENDING_NOTREADY
    #   0x7: OTA_FAILURE_POWERTRAIN_DISABLED
    #   0x8: FUNCTIONAL_SAFETY_RELEASED
    #   0x9: PREPARATION_SG
    #   0xF: Fehler
    # Valid range (raw+phys):    [0x0..0xE] 
    # Invalid range (raw+phys):  ]0xE..0xF] => [0xF..0xF]
    ...
    # Total range (raw+phys):    [0x0..0xF]
    testresult.append(testRxSigSeq(
        set_sig      = can_bus.ORU_Control_A_01__OnlineRemoteUpdateControlA__value,
        check_var    = cal.Sw_GSL_OTA_PIM_SG_ORU_Control_A_01_OnlineRemoteUpdateControlA, # Added correct cal-variable from .a2l
        set_values   = [0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9],
        check_values = [0x0, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9],
    ))
    testresult.append(["\xa0Pruefe, dass Fehlerspeicher leer ist", ""])
    ...
    '''
    # set Testcase ID #########################################################
    setTestcaseId(testresult, 'ORU_Control_A_01__ORU_Control_A_01_CRC')
    testresult.append(["\nMessage/PDU: ORU_Control_A_01:ORU_Control_A_01_CRC", ""]) 
