# *****************************************************************************
# -*- coding: latin1 -*-
# File    : FactoryMode_Write_VW_Workshop_System_Name.py
# Title   : Diagnose EGA VW Workshop System Name
# Task    : A minimal "Diagnose_EGA_VW_Workshop_System_Name!" test script
#
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_413")

    # Initialize variables ####################################################
    test_data = identifier_dict['VW Workshop System Name']
    ...
    testresult.append(canape_diag.checkResponse(vw_workshop_orig, test_data['expected_response']))
    ...
    # test step 7
    vw_workshop = [0x30, 0x30, 0x37, 0x38, 0x39]
    request = [0x2E, 0xF1, 0xAA]
    testresult.append(["[-] Schreibe VW Workshop System Name: 0x{} + 0x{}"
                       .format("".join("{:02X}".format(x) for x in request),
                               "".join("{:02X}".format(x) for x in vw_workshop)),
                       ""])
    response, result = canape_diag.sendDiagRequest(request + vw_workshop)
    ...
    testresult.append(["[.] Prüfe Inhalt: 0x{}"
                       .format(" ".join("{:02X}".format(x) for x in vw_workshop)),
                       ""])
    request = [0x22, 0xF1, 0xAA]
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    testresult.append(canape_diag.checkResponse(response[3:], vw_workshop))

    # test step 9
    request = [0x2E, 0xF1, 0xAA]
    testresult.append(["[.] Schreibe originale VW Workshop System Name: 0x{} + 0x{}"
                      .format(" ".join("{:02X}".format(x) for x in request),
                              " ".join("{:02X}".format(x) for x in vw_workshop_orig)),
                       ""])
    response, result = canape_diag.sendDiagRequest(request + vw_workshop_orig)
    ...
    # test step 10
    testresult.append(["[.] Prüfe Inhalt: 0x{}"
                       .format(" ".join("{:02X}".format(x) for x in vw_workshop_orig)),
                       ""])
    request = [0x22, 0xF1, 0xAA]
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    testresult.append(canape_diag.checkResponse(response[3:], vw_workshop_orig))
