# *****************************************************************************
# -*- coding: latin1 -*-
# File    : FactoryMode_Write_VW_Spare_Part_Number.py
# Title   : Diagnose_EGA_VW_Spare_Part_Number
# Task    : A minimal "Diagnose_EGA_VW_Spare_Part_Number!" test script
#
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_415")
    ...
    # test step 10
    testresult.append(["[.] Prüfe Inhalt: 0x{}"
                      .format(" ".join("{:02X}".format(x) for x in sparepart_number_orig)),
                       ""])
    request = [0x22, 0xF1, 0x87]
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    testresult.append(canape_diag.checkResponse(response[3:], sparepart_number_orig))
