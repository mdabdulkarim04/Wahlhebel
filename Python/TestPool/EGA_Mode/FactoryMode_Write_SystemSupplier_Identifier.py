# *****************************************************************************
# -*- coding: latin1 -*-
# File    : FactoryMode_Write_SystemSupplier_Identifier.py
# Title   : Diagnose EGA System Supplier Identifier
# Task    : A minimal "Diagnose_EGA_System_Supplier_Identifier!" test script
#
...
try:
    ...
    supplier_identifier_orig = response[3:]
    ...
    testresult.append(canape_diag.checkResponse(supplier_identifier_orig, test_data['expected_response']))
    ...
    # test step 7
    supplier_identifier = [0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 037, 0x38, 0x39]
    request = [0x2E, 0xF1, 0x8A]
    testresult.append(["[-] Schreibe System Supplier Identifier: 0x{} + 0x{}"
                       .format("".join("{:02X}".format(x) for x in request),
                               "".join("{:02X}".format(x) for x in supplier_identifier)),
                       ""])
    response, result = canape_diag.sendDiagRequest(request + supplier_identifier)
    ...
    testresult.append(["[.] Prüfe Inhalt: 0x{}"
                       .format(" ".join("{:02X}".format(x) for x in supplier_identifier)),
                       ""])
    request = [0x22, 0xF1, 0x8A]
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    testresult.append(canape_diag.checkResponse(response[3:], supplier_identifier))

    # test step 9
    request = [0x2E, 0xF1, 0x8A]
    testresult.append(["[.] Schreibe originale System Supplier Identifier: 0x{} + 0x{}"
                       .format(" ".join("{:02X}".format(x) for x in request),
                               " ".join("{:02X}".format(x) for x in supplier_identifier_orig)),
                       ""])
    response, result = canape_diag.sendDiagRequest(request + supplier_identifier_orig)
    ...
    # test step 10
    testresult.append(["[.] Prüfe Inhalt: 0x{}"
                      .format(" ".join("{:02X}".format(x) for x in supplier_identifier_orig)),
                       ""])
    request = [0x22, 0xF1, 0x8A]
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    testresult.append(canape_diag.checkResponse(response[3:], supplier_identifier_orig))
