# *****************************************************************************
# -*- coding: latin1 -*-
# File    : FactoryMode_Write_VW_FAZIT_Identification_String.py
# Title   : Diagnose EGA VW FAZIT Identification String
# Task    : A minimal "Diagnose_EGA_VW_FAZIT_Identification_String!" test script
#
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_414")
    ...
    # test step 7
    fazit_identification = [0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE]
    ...
    # test step 10
    testresult.append(["[.] Prüfe Inhalt: 0x{}"
                       .format(" ".join("{:02X}".format(x) for x in fazit_identification_orig)),
                       ""])
    request = [0x22, 0xF1, 0x7C]
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    testresult.append(canape_diag.checkResponse(response[3:], fazit_identification_orig))
