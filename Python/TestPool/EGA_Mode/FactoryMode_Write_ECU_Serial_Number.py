# *****************************************************************************
# -*- coding: latin1 -*-
# File    : FactoryMode_Write_ECU_Serial_Number.py
# Title   : Diagnose EGA ECU Serial Number
# Task    : A minimal "Diagnose_EGA_ECU_Serial_Number!" test script
#
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_406")
    ...
    testresult.append(canape_diag.checkResponse(ecu_serial_number_orig, test_data['expected_response']))
    ...
    # test step 7
    ecu_serial_number = [0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x31, 0x32]
    ...
    # test step 10
    testresult.append(["[.] Prüfe Inhalt: 0x{}"
                      .format(" ".join("{:02X}".format(x) for x in ecu_serial_number_orig)),
                       ""])
    request = [0x22, 0xF1, 0x8C]
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    testresult.append(canape_diag.checkResponse(response[3:], ecu_serial_number_orig))
