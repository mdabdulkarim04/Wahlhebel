...
# -*- coding: latin1 -*-
# File    : E2E_13.py
# Title   :ORU_Control_D_CRC_Error_First
# Task    : Erstmalige Fehlererkennung ORU_Control_D_01_CRC während "Initialized" erfolgt Wechsel in den Zustand "Safe State" innerhalb FR, wobei Flashvorgang via Überprüfung der Programmiervorbedingungen abgelehnt wird.
#
...
# ------------------------------------------------------------------------------
# 1.0  | 30.07.2022 | Devangbhai   | initial
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("E2E_13")
    ...
    # test step 1 and 2
    testresult.append(["[.] Setze ORU_Control_D_01 zyklus zeit auf 0, Schalte Kl30 ein.", ""])
    ...
    # test step 2.1
    testresult.append(["[+]  warte 640ms (Wechsel nach Initialized, Der initiale Timeout beträgt 1500ms )", ""])
    time.sleep(0.640)

    # test step 3
    testresult.append(["[-] Setze ORU_Control_D_01 zyklus zeit auf 500ms und sende ORU_Control_D_01_CRC Fehler in erste Botschaft (Wechsel nach Safe State)", ""])
    sec = 0.315
    timeout = sec + t()
