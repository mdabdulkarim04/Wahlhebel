...
# -*- coding: latin1 -*-
# File    : E2E_46.py
# Title   : ORU_Control_D_CRC_Error_SSP_Init
# Task    : Wiederholte Fehlererkennung ORU_Control_D und Wechsel nach "Safe State Permanent" aus "Initialized" über "Normal".
#
...
try:
    ...
    passiv_dtc = [(0xE0010E, 0x26)]

    # set Testcase ID #########################################################
    testresult.setTestcaseId("E2E_46")
    ...
    timeout = sec + t()
    while timeout > t():
        ORUD_timestamp = hil.ORU_Control_D_01__timestamp.get()
        hil.ORU_Control_D_01__ORU_Control_D_01_CRC__value.set(0)
        hil.ORU_Control_D_01__period.setState("an")
        new_timestamp = hil.ORU_Control_D_01__timestamp.get()
        if ORUD_timestamp != new_timestamp:
            break

    # test step 4
    testresult.append(["[.] Warte 320ms", ""])
    time.sleep(0.320)

    # test step 5
    testresult.append(["[.] Programmiervorbedingungen prüfen: 0x3101 + {}".format(HexList(test_data['identifier'])), ""])
    checkProgrammingPrecondition(exp_content=[0xA7], ticket="EGA_PRM_281")
    ...
    # test step 6
    testresult.append(["[.] Sende Zweite CRC-Error für ORU_Control_D_01_CRC. (Wechsel nach  Safe State permanant) und Warte 220ms", ""])
    sec = 0.315
    timeout = sec + t()
    while timeout > t():
        ORUD_timestamp = hil.ORU_Control_D_01__timestamp.get()
