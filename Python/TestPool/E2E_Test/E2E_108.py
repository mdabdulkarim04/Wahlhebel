...
# -*- coding: latin1 -*-
# File    : E2E_108.py
# Title   : OTAMC_D_CRC_Error_Diag_Suppressed_Repeat
# Task    : Mehrmalige Fehlererkennung OTAMC_D_01_CRC und Wechsel aus "Safe State" in den Zustand "Safe State Permanent" bei Diagnoseausblendung.
#
...
try:
    ...
    passiv_dtc = [(0xE0010F, 0x26)]

    # set Testcase ID #########################################################
    testresult.setTestcaseId("E2E_108")
    ...
    # test step 3
    testresult.append(["[.]  Warte 1000ms. ( < t = n/2 = 10/2 = 5*320 ms = 1600 ms = Zeit bis zum Wechsel in den Normalzustand, wenn kein 2. CRC Fehler erkannt wird).", ""])
    time.sleep(1.000)

    # test step 4
    testresult.append(["[.] Sende zweite CRC Failure.", ""])
    sec = 0.315
    timeout = sec + t()
    hil.OTAMC_D_01__OTAMC_D_01_CRC__value.set(0)
    hil.OTAMC_D_01__period.setState("an")
