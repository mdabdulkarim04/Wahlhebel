#******************************************************************************
# -*- coding: latin1 -*-
# File    : KeineFehlererkennung_mit_GueltigerPosition.py
# Title   : Keine Fehlererkennung mit Gueltiger Position
# Task    : Keine Fehlererkennung bei Anfahren gültiger Positionen
#
...
# 1.2  | 21.09.2021 | Devangbhai | Rework
# 1.3  | 02.03.2022 | Devangbhai patel | Rework
# 1.4  | 10.03.2022 | Devangbhai patel | Changed the waiting time
# 1.5  | 22.03.2022 | Devangbhai patel | Added the evaluation methon using the DAQ
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_15")
...
### 1 ,2
    testresult.append(["\xa0 Wählhebel langsam und kontinuierlich verfahren, nach Erreichen der jeweiligen Zielposition für mind. 100 ms verweilen:", "INFO"])
    testresult.append(["\xa0 1. Wählhebel X nach A1  bringen ", "INFO"])
    timeout = 60
    t_out = timeout + t()
    ...
    t_out2 = timeout + t()
    testresult.append(["\xa0 2. Wählhebel Von A1 nach Position X bringen", "INFO"])
    ...
    testresult.append(["\xa0 2.1 SIShift_StLghtDrvPosn = 6 setzen", "INFO"])
    descr, verdict = func_gs.changeDrivePosition('N')
    testresult.append(["\xa0" + descr, verdict])

  ### 3 ,4
    testresult.append(["\xa0 3. Wählhebel Von X nach Position B1 bringen ", "INFO"])
    timeout = 60
    t_out = timeout + t()
    ...
    t_out2 = timeout + t()
    testresult.append(["\xa0 4. Wählhebel Von B1 nach Position X bringen", "INFO"])
    ...
    testresult.append(["\xa0 4.1  SIShift_StLghtDrvPosn = 5 setzen", "INFO"])
    descr, verdict = func_gs.changeDrivePosition('D')
    testresult.append(["\xa0" + descr, verdict])

    ### 5 ,6
    testresult.append(["\xa0 5 Wählhebel von X nach A1  bringen", "INFO"])
    timeout = 60
    t_out = timeout + t()
    while hil.Waehlhebel_04__WH_SensorPos_roh__value.get() == 4 or t_out > t():
        if hil.Waehlhebel_04__WH_SensorPos_roh__value.get() == 9:
            ...
            # testresult.append(["[.] Prüfe, dass WH_Fahrstufe ist", ""])
            # testresult.append(basic_tests.checkStatus(current_status=wh_fahrstufe, nominal_status=6, descr="Prüfe, dass WH_Fahrstufe = 6 ist"))
            testresult.append(["\xa0 Wählhebel von A1 nach A2 bringen ", "INFO"])
    ...
    t_out2 = timeout + t()
    testresult.append(["\xa0 6. Wählhebel von A2 nach X bringen", "INFO"])
    ...
    testresult.append(["\xa0 6.1  SIShift_StLghtDrvPosn = 7 setzen", "INFO"])
    descr, verdict = func_gs.changeDrivePosition('R')
    testresult.append(["\xa0" + descr, verdict])

    ### 7 ,8
    testresult.append(["\xa0 7 Wählhebel von X nach B1 bringen ", "INFO"])
    timeout = 60
    t_out = timeout + t()
    while hil.Waehlhebel_04__WH_SensorPos_roh__value.get() == 4 or t_out > t():
        if hil.Waehlhebel_04__WH_SensorPos_roh__value.get() == 6:
            ...
            # testresult.append(["[.] Prüfe, dass WH_Fahrstufe ist", ""])
            # testresult.append(basic_tests.checkStatus(current_status=wh_fahrstufe, nominal_status=6, descr="Prüfe, dass WH_Fahrstufe = 6 ist"))
            testresult.append(["\xa0  Wählhebel von  B1 nach  B2 bringen ", "INFO"])
    ...
    t_out2 = timeout + t()
    testresult.append(["\xa0 8. Wählhebel  von B2 nach X bringen ", "INFO"])
    ...
    testresult.append(["\xa0 Warte 2 Sekunde, bevor DAQ Messung beendet wird", "INFO"])
    time.sleep(2)
    ...
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'x': %s" % (WH_Fahrstufe_x - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'A1': %s" % (WH_Fahrstufe_a1 - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'x': %s" % (WH_Fahrstufe_a1_x - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'B1': %s" % (WH_Fahrstufe_b1 - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'x': %s" % (WH_Fahrstufe_b1_x - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'A1': %s" % (WH_Fahrstufe_x_a1 - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'A2': %s" % (WH_Fahrstufe_a1_a2 - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'x': %s" % (WH_Fahrstufe_a2_x - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'B1': %s" % (WH_Fahrstufe_x_b1 - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'B2': %s" % (WH_Fahrstufe_b1_b2 - time_zero2), "INFO"])
    testresult.append(["Ermittelter WH_Fahrstufe Zeitpunkt 'x': %s" % (WH_Fahrstufe_b2_x - time_zero2), "INFO"])
    ...
    if WH_Fahrstufe_a1 and SensorPos_a1 is not None:
        ...
        testresult.append(basic_tests.checkRange((WH_Fahrstufe_a1 - time_zero2)- (SensorPos_a1 - time_zero), sensor_pos_wh_fahrstufe_lower, sensor_pos_wh_fahrstufe_upper, descr="Prüfe das WH fahrstufe nach 40ms nach erkennung der  SensorPos_roh = 9 gesendet wird"))
    if WH_Fahrstufe_a1 and WH_Fahrstufe_a1_x is not None:
        testresult.append(basic_tests.checkRange((WH_Fahrstufe_a1_x - time_zero2)- (WH_Fahrstufe_a1 - time_zero2), Fahrstufe_lower_limit, Fahrstufe_upper_limit, descr="Prüfe das WH fahrstufe is 6 für 50ms gesendet "))
    ...
    if SensorPos_a1_x and WH_Fahrstufe_a1_x is not None:
        ...
        testresult.append(["Wählhebel hat um %s WH_Fahrstufe = 4 gesendet " %(WH_Fahrstufe_a1_x - time_zero2), "PASSED"])
    ...
    if WH_Fahrstufe_b1 and SensorPos_b1 is not None:
        ...
        testresult.append(basic_tests.checkRange((WH_Fahrstufe_b1 - time_zero2) - (SensorPos_b1 - time_zero), sensor_pos_wh_fahrstufe_lower, sensor_pos_wh_fahrstufe_upper, descr="Prüfe das WH fahrstufe nach 40ms nach erkennung der  SensorPos_roh = 6 gesendet wird"))
    if WH_Fahrstufe_b1 and WH_Fahrstufe_b1_x is not None:
        testresult.append(basic_tests.checkRange((WH_Fahrstufe_b1_x - time_zero2) - (WH_Fahrstufe_b1 - time_zero2), Fahrstufe_lower_limit, Fahrstufe_upper_limit, descr="Prüfe das WH fahrstufe is 5 für 50ms gesendet "))
    ...
    if SensorPos_b1_x and WH_Fahrstufe_b1_x is not None:
        ...
        testresult.append(["Wählhebel hat um %s WH_Fahrstufe = 4 gesendet " % (WH_Fahrstufe_b1_x- time_zero2), "PASSED"])
    ...
    if WH_Fahrstufe_x_a1 and SensorPos_x_a1 is not None:
        ...
        testresult.append(basic_tests.checkRange((WH_Fahrstufe_x_a1 - time_zero2) - (SensorPos_x_a1 - time_zero), sensor_pos_wh_fahrstufe_lower, sensor_pos_wh_fahrstufe_upper, descr="Prüfe das WH fahrstufe nach 40ms nach erkennung der  SensorPos_roh = 9 gesendet wird"))

    if WH_Fahrstufe_x_a1 and WH_Fahrstufe_x_a1_x is not None:
        testresult.append( basic_tests.checkRange((WH_Fahrstufe_x_a1_x - time_zero2) - (WH_Fahrstufe_x_a1 - time_zero2), Fahrstufe_lower_limit, Fahrstufe_upper_limit, descr="Prüfe das WH fahrstufe is 6 für 50ms gesendet "))
    ...
    if WH_Fahrstufe_a1_a2 and SensorPos_a1_a2 is not None:
        ...
        testresult.append( basic_tests.checkRange((WH_Fahrstufe_a1_a2 - time_zero2) - (SensorPos_a1_a2 - time_zero), sensor_pos_wh_fahrstufe_lower, sensor_pos_wh_fahrstufe_upper, descr="Prüfe das WH fahrstufe nach 40ms nach erkennung der  SensorPos_roh = 7 gesendet wird"))

    if WH_Fahrstufe_a1_a2 and WH_Fahrstufe_a2_x is not None:
        testresult.append( basic_tests.checkRange((WH_Fahrstufe_a2_x - time_zero2) - (WH_Fahrstufe_a1_a2 - time_zero2), Fahrstufe_lower_limit, Fahrstufe_upper_limit, descr="Prüfe das WH fahrstufe is 7 für 50ms gesendet"))
    ...
    if SensorPos_a2_x and WH_Fahrstufe_a2_x is not None:
        ...
        testresult.append(["Wählhebel hat um %s WH_Fahrstufe = 4 gesendet " % (WH_Fahrstufe_a2_x- time_zero2), "PASSED"])
    ...
    if WH_Fahrstufe_x_b1 and SensorPos_x_b1 is not None:
        ...
        testresult.append(basic_tests.checkRange((WH_Fahrstufe_x_b1 - time_zero2) - (SensorPos_x_b1 - time_zero), sensor_pos_wh_fahrstufe_lower, sensor_pos_wh_fahrstufe_upper, descr="Prüfe das WH fahrstufe nach 40ms nach erkennung der  SensorPos_roh = 6 gesendet wird"))

    if WH_Fahrstufe_x_b1 and WH_Fahrstufe_x_b1_x is not None:
        testresult.append(basic_tests.checkRange((WH_Fahrstufe_x_b1_x - time_zero2) - (WH_Fahrstufe_x_b1 - time_zero2), Fahrstufe_lower_limit, Fahrstufe_upper_limit, descr="Prüfe das WH fahrstufe is 6 für 50ms gesendet"))
    ...
    if WH_Fahrstufe_b1_b2 and SensorPos_b1_b2 is not None:
        ...
        testresult.append( basic_tests.checkRange((WH_Fahrstufe_b1_b2 - time_zero2) - (SensorPos_b1_b2 - time_zero), sensor_pos_wh_fahrstufe_lower, sensor_pos_wh_fahrstufe_upper, descr="Prüfe das WH fahrstufe nach 40ms nach erkennung der  SensorPos_roh = 5 gesendet wird"))

    if WH_Fahrstufe_b1_b2 and WH_Fahrstufe_b2_x is not None:
        testresult.append( basic_tests.checkRange((WH_Fahrstufe_b2_x - time_zero2) - (WH_Fahrstufe_b1_b2 - time_zero2), Fahrstufe_lower_limit, Fahrstufe_upper_limit, descr="Prüfe das WH fahrstufe ist 5 für 50ms gesendet "))
    ...
    if SensorPos_b2_x and WH_Fahrstufe_b2_x is not None:
        ...
        testresult.append(["Wählhebel hat um %s WH_Fahrstufe = 4 gesendet " % (WH_Fahrstufe_b2_x- time_zero2), "PASSED"])
    ...
    if WH_Fahrstufe_error is not None:
        testresult.append(["WH_Fahrstufe ist hat  Error (15) wert gesendet um %s " %(WH_Fahrstufe_error-time_zero2), "FAILED"])
    else:
    ...
    if SensorPos_error is not None:
        testresult.append(["SensorPos_Roh ist hat  Error (15) wert gesendet um %s " %(SensorPos_error-time_zero), "FAILED"])
    else:
