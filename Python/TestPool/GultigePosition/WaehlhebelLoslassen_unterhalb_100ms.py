#******************************************************************************
# -*- coding: latin1 -*-
# File    : WaehlhebelLoslassen_unterhalb_100ms.py
# Title   : Waehlhebe lLoslassen unterhalb 100ms
# Task    : Wählhebel loslassen nach Anfahren gültiger Position unterhalb 100 ms
#
...
from global_wait_time import global_wait_time_under_debauce_counter
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_16")
    ...
    time.sleep(1)
### 1 ,2
    testresult.append(["[.] Von X nach Position A1 bringen ", ""])
    timeout = 60
    t_out = timeout + t()
    while hil.Waehlhebel_04__WH_SensorPos_roh__value.get() == 4 or t_out > t():
        if hil.Waehlhebel_04__WH_SensorPos_roh__value.get() == 9:
            testresult.append(["[.] Warte %s ms" %(global_wait_time_under_debauce_counter*1000), ""])
            ...
            time.sleep(global_wait_time_under_debauce_counter)
    ...
    t_out2 = timeout + t()
    testresult.append(["[.] Von A1 nach Position X bringen", ""])
    ...
    testresult.append(["[.] SIShift_StLghtDrvPosn = 6 setzen", ""])
    descr, verdict = func_gs.changeDrivePosition('N')
    testresult.append(["\xa0" + descr, verdict])

    ### 3 ,4
    testresult.append(["[.] Von X nach Position B1 bringen ", ""])
    timeout = 60
    t_out = timeout + t()
    ...
    t_out2 = timeout + t()
    testresult.append(["[.] Von B1 nach Position X bringen", ""])
    ...
    t_out2 = timeout + t()
    testresult.append(["[.] Wählhebel  von A2 nach X bringen", ""])
    ...
    t_out2 = timeout + t()
    testresult.append(["[.] Wählhebel  von B2 nach X bringen ", ""])
