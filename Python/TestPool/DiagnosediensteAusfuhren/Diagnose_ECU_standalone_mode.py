...
# 1.9  | 01.08.2022 | Mohammed     | Rework
...
try:
    ...
    for session in ['default', 'extended', 'programming']:
        ...
        # [response, result] = canape_diag.sendDiagRequest(request_undefined_1)
        # testresult.append(result)
        # testresult.append(canape_diag.checkNegativeResponse(response, request_undefined_1, 0x31)) ## Added Ticket ID
        ...
        testresult.append(canape_diag.checkNegativeResponse(response,
                                                            request_undefined_2,
                                                            0x7F if session == "default" else 0x31, ticket_id='Fehler Id:EGA-PRM-9'))
        # [response, result] = canape_diag.sendDiagRequest(request_undefined_2)
        # testresult.append(result)
        # testresult.append(canape_diag.checkNegativeResponse(response, request_undefined_2, 0x31,
        #                                                     ticket_id='Fehler Id:EGA-PRM-9'))  ## Added Ticket ID
        #[response, result] = canape_diag.sendDiagRequest(request_undefined_2)
    testresult.append(["[-0]", ""])
    ...
    testresult.append(["[+] Setze alle E2E Botschaft Timeout aus", ""])
