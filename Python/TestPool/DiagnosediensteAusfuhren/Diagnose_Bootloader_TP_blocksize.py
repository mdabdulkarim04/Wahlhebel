...
# 1.3  | 19.01.2022 | Mohammed     | reworked test script by update test spec
# 1.4  | 27.01.2021 | Mohammed     | reworked test script after Adding preconditions
...
try:
    ...
    test_data = identifier_dict['Bootloader TP Blocksize']
    ...
    # test step 3
    testresult.append(["[.] Diagnose Request schicken: 0x2E {} (Schreibe {})"
                       .format(HexList(test_data['identifier']), test_data['name']),
                       ""])
    request = [0x2E] + test_data['identifier']
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    ...
    testresult.append(["[.] Diagnose Request schicken: 0x2E {} (Schreibe {})"
                       .format(HexList(test_data['identifier']), test_data['name']),
                       ""])
    request = [0x2E] + test_data['identifier'] + [0x1F]
    response, result = canape_diag.sendDiagRequest(request)
    testresult.append(result)
    ...
    exp_response = [0x62] + test_data['identifier'] + [0x1F]
