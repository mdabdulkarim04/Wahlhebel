...
# 1.7  | 02.12.2021 | Mohammed     | reworked modify after testspec
# *****************************************************************************
#
# Imports #####################################################################
...
try:
    ...
    testresult.append(["[.] Prüfe Inhalt: 0x{}"
                       .format(" ".join("{:02X}".format(x) for x in sparepart_number)),
                       ""])
    ...
    # test step 9
    testresult.append(["[.] Schalte Kl.30 aus", ""])
    hil.cl30_on__.set(0)

    # test step 10
    testresult.append(["[.] Warte 500 ms und schalte Kl.30 ein", ""])
    time.sleep(.500)
    hil.cl30_on__.set(1)
    ...
    canape_diag = testenv.getCanapeDiagnostic()
    testresult.append(["\xa0 Tester Present deaktivieren", ""])
    canape_diag.disableTesterPresent()
    ...
    # test step 11
    testresult.append(["[.] Inhalt auf Korrektheit überprüfen", ""])
    testresult.append(canape_diag.checkResponse(sparepart_number_orig, test_data['expected_response'], ticket_id='FehlerId: EGA-PRM-166'))

    # test step 13
    testresult.append(["[.] Wechsel in Factory Mode: 0x1060", ""])
    testresult.extend(canape_diag.changeAndCheckDiagSession('factory_mode', read_active_session=False, ticket_id='FehlerId: EGA-PRM-166' ))

    # test step 14
    testresult.append(["[.] Auslesen der Active Diagnostic Session: 0x22F186", ""])
    testresult.extend(canape_diag.checkDiagSession('factory_mode', ticket_id='FehlerId: EGA-PRM-166'))
    ...
    testresult.append(["\xa0Überprüfen, dass Request positiv beantwortet wird", ""])
    testresult.append(canape_diag.checkPositiveResponse(response, request, ticket_id='FehlerId: EGA-PRM-166'))
