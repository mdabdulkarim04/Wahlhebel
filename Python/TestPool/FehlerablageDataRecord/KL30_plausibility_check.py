...
# -*- coding: latin-1 -*-
#
# File    : KL30_plausibility_check.py
# Title   : KL30_plausibility_check_DTC_after_3 ocy
# Task    : Set KL30 plausibility error
...
try:
    ...
    activ_dtcs = [(0x800100, 0x27)]
    pasiv_dtcs1 = [(voltage_uv['DTCs'][0], 0x2E), (0x800100, 0x2E)]
    failure_set_time = 0.1
    voltage_settle_time = 0.02  # do not know the actual time
    failure_reset_time = 0.1

    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_369")
    ...
    testresult.append(["Warte 500ms - Fehlererkennungszeit für unterspannung DTC", "INFO"])
    time.sleep(.500)
    ...
    testresult.append(["[+0]", ""])
    testresult.append(["[-] Lese Fehlerspeicher (Überspannungs-DTC Pasiv und Confirmed)", ""])

    testresult.append(canape_diag.checkEventMemory(pasiv_dtcs1, mode="ONE_OR_MORE", ticket_id='Fehler Id:EGA-PRM-275'))
