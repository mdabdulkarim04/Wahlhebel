...
# -*- coding: latin-1 -*-
#
# File    : Ablauf_Empfang_SiShift_Flg_0_nach_Start_N_Haltephase.py
# Title   : Ablauf der N Haltephase SiShift = Start-Flags
...
# ------------------------------------------------------------------------------
# 1.0  | 18.05.2021 | M. Abdul Karim | initial
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_123")
    ...
    testresult.append(["\xa03 Setze SiShift FlgStrtNeutHldPha auf 0 (StartNeutralHoldPhase)", ""])
    hil.SiShift_01__SIShift_FlgStrtNeutHldPha__value.set(0)

    testresult.append(["\xa04 Setze KL15 auf 0 (inactive) und 150ms Warten", ""])
    hil.cl15_on__.set(0)
    time.sleep(0.150)
    ...
    testresult.append(
        _checkStatus(
            current_status=hil.Waehlhebel_04__WH_Zustand_N_Haltephase_2__value,
            nominal_status=0,
            descr="Prüfe, dass Wert 0 ist", ticket_id='FehlerId:EGA-PRM-160',
        )
    )
    ...
    testresult.append(["\xa06. 1 min warten...nach 1 min CAN-Trace auswerten", ""])
    time.sleep(60)
