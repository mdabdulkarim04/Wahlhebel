...
try:
    ...
    nm_receiving_message = hil.NM_Airbag__timestamp
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId('TestSpec_111')
    ...
    testresult.append(["[.] Schalte zyklisches Senden von NM_Airbag an", ""])
    testresult.append(["Setze NM_Airbag:NM_Airbag__period auf 'an'", "INFO"])
    ...
    testresult.append(["[.] Schalte zyklisches Senden von NM_Airbag aus", ""])
    testresult.append(["Setze NM_Airbag:NM_Airbag__period auf 'aus'", "INFO"])
    ...
    if sleep_time:
        ...
        # Define Time of Prepare Bus-Sleep Mode (NM-Airbag wird nicht mehr gesendet)
        nm_Airbag_data = daq_data[str(nm_receiving_message)]
        ...
        analyse_nm_Airbag_data.seek(nm_Airbag_data['time'][rs_change_idx])
        nm_airbag_start_sending = analyse_nm_Airbag_data.findNextChanged()
        # search time, were sending stops
        _, sleep_time = func_nm.analyseCycleSleepTimes(start_time=nm_airbag_start_sending, daq_data=nm_Airbag_data)

        t_timeout_start = sleep_time + nm_airbag_start_sending - time_zero
