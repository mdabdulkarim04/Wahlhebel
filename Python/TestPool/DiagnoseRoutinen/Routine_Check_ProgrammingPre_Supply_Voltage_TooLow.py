# *****************************************************************************
# -*- coding: latin-1 -*-
# File    : Routine_Check_ProgrammingPre_Supply_Voltage_TooLow.py
# Title   : Routine Check ProgrammingPre Supply Voltage TooLow
...
# 1.2  | 19.11.2021 | Mohammed     | Rework after Isyst Review
# 1.3  | 24.11.2021 | Mohammed     | Rework
# 1.4  | 10.01.2022 | H. Förtsch   | reworked test script by test spec
# 1.5  | 14.04.2022 | Mohammed     | Added Zykluszeit
# 1.6  | 24.05.2022 | Mohammed     | Aktualisiert  Vorbedingungen
# 1.7  | 17.08.2022  | Mohammed   | Added Fehler ID
# *****************************************************************************
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_258")
    ...
    # test step 12
    testresult.append(["[.] Setze hil.vbat_cl30__V == {}V "
                       .format(vbat_cl30__V),
                       ""])
    hil.vbat_cl30__V.set(vbat_cl30__V)
    ...
    testresult.extend(canape_diag.changeAndCheckDiagSession('default', read_active_session=False, ticket_id='Fehler Id:EGA-PRM-277'))

    # test step 14
    testresult.append(["[.] Lese aktuelle Diagnose Session aus", ""])
    testresult.extend(canape_diag.checkDiagSession('default', ticket_id='Fehler Id:EGA-PRM-275'))
    ...
    # test step 17
    testresult.append(["[.] Wechsel in Extended Session: 0x1003", ""])
    testresult.extend(canape_diag.changeAndCheckDiagSession('extended', read_active_session=False, ticket_id='Fehler Id:EGA-PRM-277'))

    # test step 18
    testresult.append(["[.] Lese aktuelle Extended Session aus", ""])
    testresult.extend(canape_diag.checkDiagSession('extended', ticket_id='Fehler Id:EGA-PRM-275'))
    ...
    # test step 21
    testresult.append(["[.] Wechsel in Factory Mode: 0x1060", ""])
    testresult.extend(canape_diag.changeAndCheckDiagSession('factory_mode', read_active_session=False, ticket_id='Fehler Id:EGA-PRM-277'))

    # test step 22
    testresult.append(["[.] Lese aktuelle Factory Mode aus", ""])
    testresult.extend(canape_diag.checkDiagSession('factory_mode', ticket_id='Fehler Id:EGA-PRM-277'))
