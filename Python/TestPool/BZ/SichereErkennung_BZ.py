#******************************************************************************
# -*- coding: latin1 -*-
# File    : SichereErkennung_BZ.py
# Title   : Sichere Erkennung BZ
# Task    : Sichere Erkennung korrekter und fehlerhafter Zustände (BZ)
#
...
# 1.3  | 02.03.2022 | Devangbhai patel | Added the missing test step
# 1.4  | 10.03.2022 | Devangbhai patel | Changed the waiting time
# 1.5  | 23.05.2022 | Mohammed         | Added SiShift_01_20ms_BZ_switch variable
#******************************************************************************
# Imports #####################################################################
import time
...
try:
    ...
    # set Testcase ID #########################################################
    testresult.setTestcaseId("TestSpec_20")
    ...
    bz= hil.SiShift_01__SiShift_01_20ms_BZ__value

    meas_vars = [wh_fahrstufe, wh_sensorRoh, SiShift, bz]
...
### 1
    ...
    testresult.append(["[.] BZ der SiShift_01 Botschaft als fehlerhaft senden, SiShift_01_20ms_BZ = 1 und 200 ms warten", ""])
    timeout_2 = 0.021
    t_out = timeout_2 + t()
    while t_out > t():
        hil.SiShift_01__SiShift_01_20ms_BZ__switch.set(1)
        #hil.SiShift_01__SiShift_01_20ms_BZ__value.set(1)
        if t_out > t() == False:
            break
    time.sleep(0.500) ### todo added 500ms für SW0086
    testresult.append(["[.] Prüfe, dass WH_SensorPos_roh ist", ""])
    testresult.append(basic_tests.checkStatus(current_status=wh_sensorRoh, nominal_status=4, descr="Prüfe, dass wh_sensorRoh = 4 ist"))
    testresult.append(["[.] Prüfe, dass WH_Fahrstufe ist", ""])
    testresult.append(basic_tests.checkStatus(current_status=wh_fahrstufe, nominal_status=15,
                                              descr="Prüfe, dass WH_Fahrstufe = ERROR(15) ist"))

    testresult.append(["[.] BZ-Fehler nicht mehr senden, 500 ms warten", ""])
    ...
    plot_data = {}
    for mes in [wh_fahrstufe, wh_sensorRoh, SiShift, bz]:
