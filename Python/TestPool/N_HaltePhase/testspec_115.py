...
try:
    ...
    wait_time = 60-time_difference3
    # time.sleep(60-time_difference3)
    # descr, verdict = func_nm.waitTillTimerEnds(timer=1, switch_rx_signals=True) # removed wait time
    ...
    while t_out > t():
        ...
        if start_timestamp != curr_timestamp:
            testresult.append(["\xa0 WH starts sending the message after %sec" %(curr_timestamp-start_timestamp), "PASSED"])
            break
        elif t_out > t() == False:
            testresult.append(
    ...
    testresult += [
        ...
        _checkStatus(current_status=hil.NM_Waehlhebel__NM_Waehlhebel_NM_aktiv_Tmin__value, nominal_status=0,
                     descr="NM_aktiv_Tmin:Inaktiv ist", ticket_id='Fehler Id:EGA-PRM-15'),
        basic_tests.checkStatus(hil.NM_Waehlhebel__NM_Aktiv_N_Haltephase_abgelaufen__value, 0,
                                descr="NM_Aktiv_N_Haltephase_abgelaufen:Inaktiv")
    ]
    ##
    ...
    wait_time= 120-time_difference2+ 2
    t_out = wait_time + t()
    while t_out > t():
        curr_timestamp = nm_timestamp.get()
        if start_timestamp != curr_timestamp:
    ...
    testresult += [
        ...
        basic_tests.checkStatus(hil.Waehlhebel_04__WH_Zustand_N_Haltephase_2__value, 5,
                                descr="WH_Zustand_N_Haltephase_2: aktiv_Hinweismeldung"),
        func_nm.checkFcabBitwise(hil.NM_Waehlhebel__NM_Waehlhebel_FCAB__value.get(), [1], [],
                                 descr="NM_Waehlhebel_FCAB:12_GearSelector"),
        basic_tests.checkStatus(hil.NM_Waehlhebel__NM_Waehlhebel_NM_aktiv_Tmin__value, 1,
                                descr="NM_Waehlhebel_NM_aktiv_Tmin:Mindestaktivzeit"),
    ...
    testresult += [
        ...
        basic_tests.checkStatus(hil.Waehlhebel_04__WH_Zustand_N_Haltephase_2__value, 2,
                                descr="WH_Zustand_N_Haltephase_2: beendet_Timer_abgelaufen"),
        func_nm.checkFcabBitwise(hil.NM_Waehlhebel__NM_Waehlhebel_FCAB__value.get(), [1], [],
                                 descr="NM_Waehlhebel_FCAB:12_GearSelector"),
        _checkStatus(current_status=hil.NM_Waehlhebel__NM_Waehlhebel_NM_aktiv_Tmin__value, nominal_status=1,
                     descr="NM_aktiv_Tmin:Mindestaktivzeit ist", ticket_id='Fehler Id:EGA-PRM-15'),
