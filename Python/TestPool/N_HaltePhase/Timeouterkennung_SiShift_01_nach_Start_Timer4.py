...
# 1.10 | 26.07.2022 | Mohammed    | test step 3 und 7 Testschritte aktualisiert
...
try:
    ...
    while t_out > t():
        ...
        if start_timestamp != curr_timestamp:
            testresult.append(
                ["\xa0 WH starts sending the message after %s microsec" % (curr_timestamp - start_timestamp), "INFO"])
            WH_Sends_data = True
            break
        elif t_out > t() == False:
            testresult.append(
    ...
    # check_cycletime(sec=0.140, nm_msg=True)
    ...
    testresult += [
        ...
        basic_tests.checkStatus(hil.NM_Waehlhebel__NM_Waehlhebel_CBV_AWB__value, 1, descr="Prüf dass NM_Waehlhebel_CBV_AWB= 1"),
        basic_tests.checkStatus(hil.NM_Waehlhebel__NM_Aktiv_N_Haltephase_abgelaufen__value, 0, descr="Prüf dass NM_Aktiv_N_Haltephase_abgelaufen= 0"),
        _checkStatus(hil.Waehlhebel_04__WH_Zustand_N_Haltephase_2__value, 5, descr="Prüf dass WH_Zustand_N_Haltephase_2 = 5", ticket_id='Fehler-Id: EGA-PRM-232')
    ]
