...
# Task    : Wrapper for Interface to the dSPACE XIL-2.x-Testbench implementation.
#           All .NET bindings are wrapped to provide a plain Python interface.
#            
#           This serves as "interface" to the precompiled module in delivery to 
#           enable code-completion in PyDev.
#
#           Requires "Python for .NET" (pythonnet) for .NET access
#           (which is provided in dSPACE releases that have XIL-API support)
#
# Type    : Interface
# Python  : 2.7
#
...
#------------------------------------------------------------------------------
# 1.0  | 11.04.2016 | J.Tremmel  | initial, for _xil_api v1.5 
# 1.1  | 18.10.2016 | J.Tremmel  | moved shared error classes to xil_api_common
# 1.2  | 27.09.2018 | J.Tremmel  | added listing of available assemblies to __main__,
#                                | updated default task_name used for Capture 
# 1.3  | 17.10.2018 | J.Tremmel  | added import of (.NET) TimeoutException
# 1.4  | 22.06.2020 | J.Tremmel  | minor formatting tweaks, added x64 support note
#******************************************************************************
"""
@package ttk_tools.dspace.xil_api
Wrapper for interface to the dSPACE XIL-2.x-Testbench implementation in 
ttk_tools.dspace._xil_api.
All .NET bindings are wrapped to provide a plain (and somewhat less obnoxious) 
Python interface.
...
Note:
   ...
   Current dSPACE XIL-2.x-Testbench implementations might only be available 
   for x64.

"""
import _xil_api

# enums (just to make them available) # # # # # # # # # # # # # # # # # # # # # 
from _xil_api import SignalGeneratorState   # @UnusedImport
from _xil_api import DataType               # @UnusedImport
from _xil_api import InterpolationTypes     # @UnusedImport
from _xil_api import CaptureState           # @UnusedImport
from _xil_api import MAPortState            # @UnusedImport
...
# Exceptions from XIL-API Testbench layer. 
# Hierarchy: 
# Exception -> (.NET) System.Exception -> TestbenchPortException -> MAPortError

## plain .NET System.Exceptions
from _xil_api import DotNetException        # @UnusedImport @UnresolvedImport
from _xil_api import TimeoutException       # @UnusedImport @UnresolvedImport
from _xil_api import TestbenchPortException # @UnusedImport @UnresolvedImport
# Note that MAPortError is derived from TestbenchPortException
# and TimeoutException is derived from DotNetException

# utility function(s)
from xil_api_common import writeMAPortConfigXML  # @UnusedImport


# #############################################################################
# Testbench
# #############################################################################
class XilTestbench(_xil_api.XilTestbench):
    """ XilTestbench access """
    ...
    def __init__(self, 
        ...
        """ Setup a dSPACE XIL testbench along with a MAPort.
        ...
        # will be initialized in parent constructor
        self.testbench = None 
        ## [XIL-API] model access port, 
        # will be initialized in parent constructor
        self.ma_port = None
        
        _xil_api.XilTestbench.__init__(
            self, 
    ...
    def dispose(self):
        ...
        """
        _xil_api.XilTestbench.dispose(self)
    ...
    def read(self, model_path):
        ...
        """
        return _xil_api.XilTestbench.read(self, model_path)
    ...
    def write(self, model_path, value):
        ...
        """
        return _xil_api.XilTestbench.write(self, model_path, value)
    
    # #########################################################################
    def getCapture(self):
        """ Get a Capture instance configured to use this XilTestbench. """
        return _xil_api.XilTestbench.getCapture(self)
    
    # #########################################################################
    def getStimulus(self):
        """ Get a Stimulus instance configured to use this XilTestbench. """
        return _xil_api.XilTestbench.getStimulus(self)
        

# #############################################################################
# Capture / Data Acquisition
# #############################################################################
class Capture(_xil_api.Capture):
    """ Capture / Data Acquisition """
    # #########################################################################
    def __init__(self, xil_testbench):
        ...
        ## result writer instance to use for writing captured data
        # (configured during setup())
        self.result_writer = None
        
        _xil_api.Capture.__init__(self, xil_testbench)
    ...
    def dispose(self):
        """ Dispose of resources allocated by this Capture instance.
            ...
            Note:
                After this method has been called, further access to capture 
                data will fail.
        """
        _xil_api.Capture.dispose(self)
    
    # #########################################################################
    def start(self):
        """ Start capturing. 
            
            Attention: stream-to-disk:  
                start() will raise a (somewhat generic) `TestbenchPortException` 
                ("Could not start capturing.") if the configured MDF-file is 
                locked by another application.
        """
        _xil_api.Capture.start(self)
    
    # #########################################################################
    def stop(self):
        """ Stop capture process """
        _xil_api.Capture.stop(self)
    ...
    def fetch(self):
        ...
        """
        return _xil_api.Capture.fetch(self)
    ...
    def getData(self, verbosity=1):
        ...
        """
        return _xil_api.Capture.getData(self, verbosity)
        

# #############################################################################
# 
# #############################################################################
class StimulusSignalDescription(_xil_api.StimulusSignalDescription):
    """ Signal value description for stimulus / signal generator.
        Describes signal value changes over time.
    """
    # #########################################################################
    def __init__(self, xil_testbench, name, model_path):
        """ Stimulus signal description.
        ...
        _xil_api.StimulusSignalDescription.__init__(
    ...
    def addConstSegment(self, duration, value):
        ...
        """
        _xil_api.StimulusSignalDescription.addConstSegment(self, duration, value)
    ...
    def addRampSegment(self, duration, start_value, stop_value):
        ...
        """
        _xil_api.StimulusSignalDescription.addRampSegment(
    ...
    def addIdleSegment(self, duration):
        """ Add an `IdleSegment`. This sets `sig_descr` generation into idle-mode 
            ...
            Parameters:
                duration - duration of segment in [s]
        """
        _xil_api.StimulusSignalDescription.addIdleSegment(self, duration)
    ...
    def addNoiseSegment(self, duration, mean, sigma, seed=None):
        ...
        """
        _xil_api.StimulusSignalDescription.addNoiseSegment(
    ...
    def addRampSlopeSegment(self, duration, offset, slope):
        ...
        """
        _xil_api.StimulusSignalDescription.addRampSlopeSegment(
    ...
    def addSineSegment(self, duration, offset, amplitude, period, phase=0):
        ...
        """
        _xil_api.StimulusSignalDescription.addSineSegment(
    ...
    def addSawSegment(self, duration, offset, amplitude, period, phase=0, duty_cycle=0.5):
        ...
        """
        _xil_api.StimulusSignalDescription.addSawSegment(
    ...
    def addPulseSegment(self, duration, offset, amplitude, period, phase=0, duty_cycle=0.5):
        ...
        """
        _xil_api.StimulusSignalDescription.addPulseSegment(
            self, duration, offset, amplitude, period, phase, duty_cycle
        )
    
    # #########################################################################
    def addExpSegment(self, duration, start_value, stop_value, tau):
        """ Add a `ExpSegment`, a segment following an exponential curve.
        ...
        """
        _xil_api.StimulusSignalDescription.addExpSegment(
            ...
            Parameters:
                ...
                interpolation - interpolation method:
                                ...
                                * "forward":  next data point will be used immediately 
                                              (staircase forward)
        """
        _xil_api.StimulusSignalDescription.addSignalValueSegment(
    ...
    def addDataFileSegment(self, file_path, 
        ...
        """
        _xil_api.StimulusSignalDescription.addDataFileSegment(
            self, file_path, 
...
# 
# #############################################################################
class Stimulus(_xil_api.Stimulus):
    ...
    def __init__(self, xil_testbench, appl_name=None):
        ...
        _xil_api.Stimulus.__init__(self, xil_testbench, appl_name)
    ...
    def loadStimulusFile(self, file_path, assignments=None):
        ...
        """
        _xil_api.Stimulus.loadStimulusFile(self, file_path, assignments)
    ...
    def addSignalDescription(self, stim_sig_descr):
        ...
        """
        _xil_api.Stimulus.addSignalDescription(self, stim_sig_descr)
        
    
    # #########################################################################
    def loadToTarget(self):
        """ Load stimulus to target application """
        _xil_api.Stimulus.loadToTarget(self)
    ...
    def destroyOnTarget(self):
        """ Destroy (well, unload) a currently loaded stimulus on/from target 
            application.
        """
        _xil_api.Stimulus.destroyOnTarget(self)
    
    # #########################################################################
    def start(self):
        """ Start a loaded stimulus. """
        _xil_api.Stimulus.start(self)
    
    # #########################################################################
    def pause(self):
        """ Pause a running stimulus. """
        _xil_api.Stimulus.pause(self)
    
    # #########################################################################
    def stop(self):
        """ Stop a running (or paused) stimulus. """
        _xil_api.Stimulus.stop(self)
    ...
    def getState(self, verbosity=1):
        ...
        """
        return _xil_api.Stimulus.getState(self, verbosity)
    ...
    def getStateText(self, state=None):
        """ Get a string description for a numeric stimulus state.
            ...
            Returns a string.
        """
        return _xil_api.Stimulus.getStateText(self, state)
    
    # #########################################################################
    def isRunning(self):
        """ Return True if signal generation is currently running. """
        return _xil_api.Stimulus.isRunning(self)
    
    # #########################################################################
    def isDone(self):
        """ Return True if signal generation is done (finished or stopped)."""
        return _xil_api.Stimulus.isDone(self)
    
    # #########################################################################
    def getElapsedTime(self):
        """ Get elapsed time in seconds. """
        return _xil_api.Stimulus.getElapsedTime(self)
    ...
    def waitUntilDone(self, timeout, step_size=0.500, auto_destroy=True, verbosity=2):
        ...
        """
        _xil_api.Stimulus.waitUntilDone(
...
if __name__ == '__main__':  # pragma: no cover (contains only sample code)
    import clr
    print "# .NET Assemblies: ################################################"
    for index, assembly in enumerate(clr.ListAssemblies(True), 1):
        print "%3d. %s"%(index, assembly)
